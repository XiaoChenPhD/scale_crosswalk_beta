# Benchmarking all available models to link HAMD scores 
# and MADRS scores using different time points.
# From HAMD to MADRS, modified from "do_primary_analysis.Rmd"
# Xiao Chen 240422
#
# Modified 240603
# save some redundant codes as functions and stop including any covariates in the models since they 
# are not helpful
# 
# Modified 240729 beta version
# trying using the FOURD screen data, its missing rate is lower
#
# Modified 240819
# carry over the screen data to the baseline data if baseline data is missing in FOURD dataset
#
# Modified 240822
...
model_current_prefix <- "data_model_ALL_carryover_"
...
row_names <- c('Intercept', 'HRSD sum score')
...
row_names <- c('Intercept', 'HRSD item 1', 'HRSD item 2', 'HRSD item 3', 'HRSD item 4', 
               'HRSD item 5', 'HRSD item 6', 'HRSD item 7', 'HRSD item 8', 'HRSD item 9',
               'HRSD item 10', 'HRSD item 11', 'HRSD item 12', 'HRSD item 13', 'HRSD item 14', 
               'HRSD item 15', 'HRSD item 16', 'HRSD item 17')
column_names <- c('Baseline', 'T30', 'Follow-up 1', 'Follow-up 2', 'Follow-up 3', 'Delta')
...
for (flag in c(1,2,3,4,5,6)){
  ...
  predictions <- c()
  for (i in seq_along(validation_sample$hrsd_total)){
    if (validation_sample$hrsd_total[[i]] < -5){
      # the original conversion table only listed delta_HAMD >= -5
      predictions <- c(predictions, -8)
    }else{
      predictions <- c(predictions, 
            conversion_table$MADRS[match(validation_sample$hrsd_total[[i]], conversion_table$HAMD)])
    }
  }
  
  rmse_b <- NA
  mae_b <- NA
  rmse <- rmse(validation_sample$madrs_total, predictions)
  mae <- mae(validation_sample$madrs_total, predictions)
  ...
  y <- as.freqtab(table_madrs_total)
  table_hrsd_total <- table(derivative_sample$hrsd_total)
  x <- as.freqtab(table_hrsd_total)
  ...
  predictions <- equate(validation_sample$hrsd_total, y = equating_result)
  # benchmarking
  rmse <- rmse(validation_sample$madrs_total, predictions)
  mae <- mae(validation_sample$madrs_total, predictions)
  ...
  if(flag != 1){
    ...
    predictions_b <- equate(validation_sample$hrsd_total, y = equating_result_b)
    rmse_b <- rmse(validation_sample$madrs_total, predictions_b)
    mae_b <- mae(validation_sample$madrs_total, predictions_b)
  }
  ...
  model_lm <- lm(madrs_total ~ hrsd_total, data = derivative_sample)
  ...
  model_lm_item <- lm(madrs_total ~ hrsd1s + hrsd2s + hrsd3s + hrsd4s + hrsd5s 
                      + hrsd6s + hrsd7s + hrsd8s + hrsd9s + hrsd10s + hrsd11s + 
                        hrsd12s + hrsd13s + hrsd14s + hrsd15s + hrsd16s + hrsd17s, 
                      data = derivative_sample)
  ...
  model_rf <- randomForest(madrs_total ~ hrsd_total, data = derivative_sample, ntree = 500)
  ...
  model_rf_item <- randomForest(madrs_total ~ hrsd1s + hrsd2s + hrsd3s + hrsd4s + 
                                  hrsd5s + hrsd6s + hrsd7s + hrsd8s + hrsd9s + 
                                  hrsd10s + hrsd11s + hrsd12s + hrsd13s + hrsd14s + 
                                  hrsd15s + hrsd16s + hrsd17s, data = derivative_sample, ntree = 500)
  ...
  ######################### svm regression #####################################
  x <- select(derivative_sample, c(hrsd_total))
  x_test <- select(validation_sample, c(hrsd_total))
  y <- derivative_sample$madrs_total
  ...
  ######################### svm regression item #################################
  x <- select(derivative_sample, c(hrsd1s, hrsd2s, hrsd3s, hrsd4s, hrsd5s, hrsd6s, 
                                   hrsd7s, hrsd8s, hrsd9s, hrsd10s, hrsd11s, hrsd12s, 
                                   hrsd13s, hrsd14s, hrsd15s, hrsd16s, hrsd17s))
  y <- derivative_sample$madrs_total
  x_test <- select(validation_sample, c(hrsd1s, hrsd2s, hrsd3s, hrsd4s, hrsd5s, 
                                        hrsd6s, hrsd7s, hrsd8s, hrsd9s, hrsd10s, 
                                        hrsd11s, hrsd12s, hrsd13s, hrsd14s, 
