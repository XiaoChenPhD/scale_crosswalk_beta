...
custom_colors <- c("Leucht 2018" = "#6C946F", "Linear Regression" = "#D4BDAC", "Percentile" = "#FFA823", "Random Forest Regression" = "#DC0083", "SVM Regression" = "#536493", "SVM Regression Item" = "#8CB9BD", "SVM Regression Cov" = "#E7F0DC", "SVM Regression Item Cov" = "#B6C7AA")
```
...
rownames(data_matrix) <- c("Leucht 2018", "Percentile", "Linear Regression", "Random Forest Regression", "SVM Regression")
...
data_matrix <- matrix(0, nrow = 5, ncol = 3)
colnames(data_matrix) <- c("Followup 1", "Followup 2", "Followup 3")
rownames(data_matrix) <- c("Leucht 2018", "Percentile", "Linear Regression", "Random Forest Regression", "SVM Regression")
...
df_full <- rbind(df_H2M,df_M2H)
selected_columns = df_full[, c("Followup 1", "Followup 2", "Followup 3")]
max_value <-  max(selected_columns)
...
df_long <- df_H2M %>% 
  pivot_longer(cols = c("Followup 1", "Followup 2", "Followup 3"),
               names_to = "Time point",
               values_to = "RMSE")
df_long$`Time point` <- factor(df_long$`Time point`, levels = c("Followup 3", "Followup 2", "Followup 1"), ordered = TRUE)
...
colnames(data_matrix) <- c("T30", "Followup 1", "Followup 2", "Followup 3", "Delta")
rownames(data_matrix) <- c("Percentile", "Linear Regression", "Random Forest Regression", "SVM Regression")
...
df_full <- rbind(df_H2M,df_M2H)
selected_columns = df_full[, c('T30', 'Followup 1', 'Followup 2', 'Followup 3', 'Delta')]
max_value <-  max(selected_columns)
...
df_long <- df_H2M %>% 
  pivot_longer(cols = c('T30', 'Followup 1', 'Followup 2', 'Followup 3', 'Delta'),
               names_to = "Time point",
               values_to = "RMSE")
df_long$`Time point` <- factor(df_long$`Time point`, levels = c("Delta", "Followup 3", "Followup 2", 'Followup 1', 'T30'), ordered = TRUE)
...
colnames(data_matrix) <- c("Baseline", "T30", "Delta")
rownames(data_matrix) <- c("Percentile", "SVM Regression", "SVM Regression Item")
